    ...
    <ul class="individualUserBg">
        <form id="regForm_mod" name="regForm_mod" method="post"  >
			...
			<span  class="regM" style="margin-left:29px">
				...
				<a href="https://passport.e3mall.cn/xy.html" target="_blank"  class="checkTitle">我已阅读并同意<font style="font-size:12px;">《GODV商城用户注册协议》</font></a>
			</span>
...
<!--mainOver-->
		...
		<div class="footer">
			...
			<p> 
				...
				Copyright© GODV商城 e3mall.cn 版权所有<br /> 
			</p>
		</div>
		<!-- /footer -->
<script type="text/javascript">
	var REGISTER={
		...
		beforeSubmit:function() {
				//检查用户是否已经被占用
				$.ajax({
	            	url : REGISTER.param.surl + "/user/check/"+escape($("#regName").val())+"/1.action?r=" + Math.random(),
	            	success : function(data) {
	            		if (data.data) {
	            			//检查手机号是否存在
	            			$.ajax({
	            				url : REGISTER.param.surl + "/user/check/"+$("#phone").val()+"/2.action?r=" + Math.random(),
				            	success : function(data) {
				            		if (data.data) {
					            		REGISTER.doSubmit();
				            		} else {
		...
		doSubmit:function() {
			$.post("/user/register.action",$("#regForm_mod").serialize(), function(data){
				if(data.status == 200){
